...
import com.example.schoolapp.databinding.FragmentFirstBinding
...
class FirstFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val adapter = SchoolListAdapter{school ->
            val action = FirstFragmentDirections.actionFirstFragmentToSecondFragment(school)
            findNavController().navigate(action)
        }
        binding.dataRecycleView.adapter = adapter
        binding.dataRecycleView.layoutManager = LinearLayoutManager(context)
        schoolViewModel.allSchool.observe(viewLifecycleOwner) { school ->
            school.let {
                if (school.isEmpty()) {
